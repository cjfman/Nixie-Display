...
_CALIB_DEBOUNCE   = 0.2  ## minimum seconds between accepted taps; blocks hold-repeat (~33ms)
...
def _build_items(draft):
    ...
    return [
        ...
        _s_entry('invert_v', 'bool',
                 lambda d: "INVERT V " + ("ON" if d.get('invert_v', True) else "OFF"), "INVERT V",
                 lambda d: d.get('invert_v', True),
                 lambda d, v: d.__setitem__('invert_v', v)),
        _s_entry('invert_h', 'bool',
                 lambda d: "INVERT H " + ("ON" if d.get('invert_h', True) else "OFF"), "INVERT H",
                 lambda d: d.get('invert_h', True),
                 lambda d, v: d.__setitem__('invert_h', v)),
...
class TapRevolutionCalibrationItem(MenuItem):
    ...
    def _tap(self):
        ...
        if self._last_tap_when is not None and when - self._last_tap_when < _CALIB_DEBOUNCE:
            return
